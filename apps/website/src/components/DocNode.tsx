...
export function DocNode({ node, version }: { readonly node?: any; readonly version: string }) {
		...
		switch (node.kind) {
			...
			case 'LinkTag': {
				if (node.resolvedPackage) {
					return (
						<Link
							key={`${node.resolvedPackage.packageName}-${idx}`}
							className="font-mono text-blurple hover:text-blurple-500 dark:hover:text-blurple-300"
							href={`/docs/${node.resolvedPackage.packageName}/${version}/${node.uri}`}
						>
							{node.text}
						</Link>
					);
			...
			case 'FencedCode': {
				...
				return (
					<OverlayScrollbarsComponent
						...
						className="my-4 rounded-md border border-neutral-300 dark:border-neutral-700"
					>
						<SyntaxHighlighter className="py-3 text-sm" lang={language} code={text} />
					</OverlayScrollbarsComponent>
				);
			}
