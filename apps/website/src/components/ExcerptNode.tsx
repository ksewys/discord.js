...
export function ExcerptNode({ node, version }: { readonly node?: any; readonly version: string }) {
	const createExcerpt = (excerpts: any) => {
		const excerpt = excerpts.excerpts ?? [excerpts];
		...
		return (
			...
			>
				{excerpt.map((excerpt: any, idx: number) => {
					if (excerpt.resolvedItem) {
						return (
							<Link
								...
								href={`/docs/${excerpt.resolvedItem.packageName}/${version}/${excerpt.resolvedItem.uri}`}
							>
								{excerpt.text}
							</Link>
						);
					...
					return <span key={`${excerpt.text}-${idx}`}>{excerpt.text}</span>;
				})}
			</span>
		);
	};
